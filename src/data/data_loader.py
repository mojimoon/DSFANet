...
class DataPreprocessor:
    ...
    def clean_data(self, df):
        """Replace invalid numeric values and drop high-cardinality IP identifiers."""
        df = df.copy()
        df.replace([np.inf, -np.inf], np.nan, inplace=True)
        df.fillna(0, inplace=True)

        cols_to_drop = ["IPV4_SRC_ADDR", "IPV4_DST_ADDR"]
        existing_cols = [column for column in cols_to_drop if column in df.columns]
    ...
    def prepare_data(self):
        ...
        if config.LABEL_COLUMN not in df.columns:
            for candidate in ["Label", "label", "Attack", "attack", "y"]:
                if candidate in df.columns:
                    print(f"Renaming column '{candidate}' to '{config.LABEL_COLUMN}'")
                    df.rename(columns={candidate: config.LABEL_COLUMN}, inplace=True)
                    break
        ...
        static_cols = [column for column in config.STATIC_FEATURES if column in df.columns]
        temporal_cols = [column for column in config.TEMPORAL_FEATURES if column in df.columns]
        ...
        x_static = (
            df[static_cols].apply(pd.to_numeric, errors="coerce").fillna(0).values.astype(np.float32)
        )
        x_temporal = (
            df[temporal_cols].apply(pd.to_numeric, errors="coerce").fillna(0).values.astype(np.float32)
        )
